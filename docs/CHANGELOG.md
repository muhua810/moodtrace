...
### 🐛 Bug 修复
- **修复纠正按钮在编辑模式下不可见**：RecordPage 纠正按钮条件 `!saved` 导致查看已保存记录时无法触发纠正功能。改为 `(!saved || editing)`，进入编辑模式后即可看到纠正按钮
...
### ✨ 新功能
- **AI 纠正反馈学习**：记录页新增情绪纠正按钮，用户可手动选择正确情绪类别修正 AI 分析结果。调用 `learnFromFeedback(text, correctIntensity)` 实现增量学习，动态调整关键词权重。纠正后 confidence 置为 1.0，method 标记为 `corrected`
