...
- 🤖 **AI 情绪分析** — 四层降级策略 + 用户纠正反馈学习，零配置即可使用，详见 [AI 分析架构](#-ai-分析架构)
...
| 测试 | Vitest + Testing Library | 191 用例，核心模块 90%+ 覆盖 |
...
- **关键词引擎**：分句 → 否定词检测 → 反讽检测 → emoji 加权 → 加权平均，覆盖 193 个中英文关键词
- **统计分析器**：字符 bigram 分词 → TF-IDF 特征提取 → 朴素贝叶斯分类，200 条训练语料
- **用户纠正反馈**：用户可手动纠正 AI 判定的情绪类别，系统通过 `learnFromFeedback` 增量学习调整关键词权重，持续提升准确率
...
|------|:----------:|:----:|:-----:|:----:|
| 使用门槛 | 一句话即可 | 图标选择 | 问卷式 | 多选标签 |
| AI 分析 | ✅ 四层降级 | ❌ 手动选择 | 有限（仅英文） | ❌ 手动选择 |
| 隐私保护 | 纯前端 + 加密 | 本地+云混合 | 云端存储 | 云端+账号 |
| 离线使用 | ✅ PWA 支持 | ✅ | ❌ | ❌ |
| 可视化 | 热力图 + 多维统计 | 基础日历 | 周报 PDF | 无情绪图表 |
| 多语言 | 6 种语言 | 30+ | 2 | 1 |
| 开源 | ✅ MIT | ❌ | ❌ | ❌ |
